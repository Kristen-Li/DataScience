...
%     k = pcak(k_list,train_set(:,[1:end-2,end]),0.2);
   ...
   % F_pca_new=Fhat_pca_all(end-size(x_test,1)+1:end,1:k);
    ...
    m.out(i+num:i+num+size(test_set,1)-1, 1) = y_hat_pca_temp;
    ...
    fvu_pca_top(i,:) = sum((table(end-499:end,2)-table(end-499:end,1)).^2)/sum(y_test.^2);
...
%% save results
clear Z train_set y_train x_test y_test Fhat_pca Fhat_pca_all predictor_pca;
mkdir('stocks_output')
